...
#########           Updated by Justin Gourley on 08-02-10        #########
##########################################################################

include_once(dirname(__FILE__) . "/xmlrpc-2.0/lib/xmlrpc.inc");
...
###Connect to internal apps###
public function intCon($name,$dbOn="on") {
  $this->debug = $dbOn;

	$params = JComponentHelper::getParams('com_infusiongroup');
	$applicationName = $params->get('applicationName');
	$this->key = $params->get('applicationKey');
	if(empty($applicationName) || empty($this->key)) {
		return false;
	}

	$this->client = new xmlrpc_client("https://".$applicationName.":/api/xmlrpc");
...
public function cfgCon($name,$dbOn="on") {
  $this->debug = $dbOn;
	...
	$this->client = new xmlrpc_client("https://".$applicationName.".infusionsoft.com/api/xmlrpc");
...
/*
public void setInvoiceSyncStatus(int id, boolean syncStatus);
public void setPaymentSyncStatus(int id, boolean syncStatus);
public String getPluginStatus(String fullyQualifiedClassName);
public List getAllShippingOptions();
public Map getAllPaymentOptions();
...
public function commOverride($invId,$affId,$prodId,$percentage,$amt,$payType,$desc,$date) {
    ...
    $carray = array(
        php_xmlrpc_encode($this->key),
        php_xmlrpc_encode($invId),
	  php_xmlrpc_encode($affId),
        php_xmlrpc_encode($prodId),
...
public function blankOrder($conId,$desc,$oDate,$leadAff,$saleAff) {
    ...
    $carray = array(
        ...
        php_xmlrpc_encode($leadAff),
        php_xmlrpc_encode($saleAff));
...
public function updateSubscriptionNextBillDate($subscriptionId,$nextBillDate) {
    ...
    $carray = array(
                    php_xmlrpc_encode($this->key),
                    php_xmlrpc_encode($subscriptionId),
...
public function attachEmail($cId, $fromName, $fromAddress, $toAddress, $ccAddresses,
			    ...
			    $header, $strRecvdDate, $strSentDate) {
...
public function createEmailTemplate($title, $userID, $fromAddress, $toAddress, $ccAddresses, $bccAddresses, $contentType, $subject, $htmlBody, $txtBody) {
    echo "HI!!!";
    $carray = array(
        php_xmlrpc_encode($this->key),
...
public function getEmailTemplate($templateId) {
  $carray = array(php_xmlrpc_encode($this->key), php_xmlrpc_encode($templateId));
